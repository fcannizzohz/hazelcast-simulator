...
`/workspace`, reuses the host `~/.m2` cache, and persists AWS, Google Cloud, and
Kubernetes client configuration on the host. It rejects projects outside the
workspace so a command cannot accidentally mount an unrelated directory.
...
Docker must be available, and `~/.m2` must exist and be readable and writable
by the user running Docker. Cloud credentials remain in their normal host
locations and are mounted only when the launcher runs.
