...
resource "aws_instance" "nodes" {
  ...
  user_data = <<-EOL
    ...
    mkdir /home/ubuntu/tiered-store
    mount /dev/nvme0n1 /home/ubuntu/tiered-store
    chown -R ubuntu:ubuntu /home/ubuntu/tiered-store
    EOL
}
...
resource "aws_security_group" "observability-sg" {
  name        = "simulator-security-group-observability-${local.settings.basename}"
  description = "Security group for Prometheus and Grafana"
  vpc_id      = local.settings.vpc_id

  tags = merge({
    Name = "Simulator Observability Security Group ${local.settings.basename}",
  }, local.common_resource_tags)
...
resource "aws_instance" "observability" {
  key_name               = aws_key_pair.keypair.key_name
  ami                    = local.settings.observability.ami
  instance_type          = local.settings.observability.instance_type
  count                  = local.settings.observability.count
  subnet_id              = aws_subnet.subnet.id
  availability_zone      = local.settings.availability_zone
  ...
  tags = merge({
    Name                                       = "Simulator Observability ${local.settings.basename}"
    "passthrough:ansible_ssh_private_key_file" = local.settings.keypair.private_key
